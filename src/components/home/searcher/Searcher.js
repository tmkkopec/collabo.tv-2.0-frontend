...
export default class Searcher extends Component {
    ...
    nextPage(event){
        fetch('https://www.googleapis.com/youtube/v3/search?part=snippet&q='+this.state.lastQuery+'&pageToken='+
            ...
            .then(d => {
                if(d.nextPageToken){
                    this.setState({response: d, nextActive: true, next: d.nextPageToken, prevActive: true,
                    prev: d.prevPageToken})
                } else{
                    this.setState({response: d, nextActive: false, next: null, prevActive: true,
                        prev: d.prevPageToken})
                }
            })
    }
        ...
        fetch('https://www.googleapis.com/youtube/v3/search?part=snippet&q='+this.state.lastQuery+'&pageToken='+
            ...
            .then(d => {
                if(d.prevPageToken){
                    ...
                    this.setState({response: d, nextActive: true, next: d.nextPageToken, prevActive: false,
                        prev: null})
                }
            })
    }
    ...
    onSubmit(event) {
        if (this.state.query !== '') {
            fetch('https://www.googleapis.com/youtube/v3/search?part=snippet&q='+this.state.query+'&type=video&key=AIzaSyAkWmNO7E-FZWXxsNrD9nqsKTdvJpNti2I')
                ...
                .then(d => {
                    if(d.nextPageToken){
                        this.setState({lastQuery: this.state.query, resultsVisible: true, response: d, nextActive: true, next: d.nextPageToken,
                        prevActive: false, prev: null})
                    }else{
                        this.setState({lastQuery: this.state.query, resultsVisible: true, response: d, nextActive: false, next: null,
                            prevActive: false, prev: null})
                    }
